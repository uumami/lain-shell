...
lain-core SHALL create a PTY via portable-pty and spawn the user's default shell (from `$SHELL` or fallback to `/bin/sh`). The PTY SHALL be handed to alacritty_terminal's `EventLoop` for reading and VTE parsing.
...
- **WHEN** the user types `echo $TERM` in the spawned shell
